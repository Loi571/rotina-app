  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center p-6">
      <div className="bg-gray-800 border border-gray-700 rounded-xl p-8 space-y-4 w-full max-w-md">
        ...
        <button onClick={handleSignup} className="bg-indigo-600 hover:bg-indigo-500 px-4 py-2 rounded w-full transition-all">Cadastrar</button>
        {error && <p className="text-red-400 text-sm">{error}</p>}
        <p className="text-sm text-gray-400">Já tem conta? <a href="/login" className="text-indigo-400">Entrar</a></p>
      </div>
    </div>
  )
}